...
void eval_spec_rates (const double * __restrict__ fwd_rates, const double * __restrict__ rev_rates, const double * __restrict__ pres_mod, double * __restrict__ sp_rates, double * __restrict__ dy_N) {
  ...
  //sp 3
  sp_rates[3] -= (fwd_rates[6] - rev_rates[6]) * pres_mod[3];
  //sp 6
  sp_rates[6] += (fwd_rates[6] - rev_rates[6]) * pres_mod[3];

  //rxn 7
  //sp 1
  sp_rates[1] -= (fwd_rates[7] - rev_rates[7]) * pres_mod[4];
  //sp 3
  sp_rates[3] -= (fwd_rates[7] - rev_rates[7]) * pres_mod[4];
  //sp 6
  sp_rates[6] += (fwd_rates[7] - rev_rates[7]) * pres_mod[4];

  //rxn 8
  //sp 1
  sp_rates[1] -= (fwd_rates[8] - rev_rates[8]) * pres_mod[5];
  //sp 3
  sp_rates[3] -= (fwd_rates[8] - rev_rates[8]) * pres_mod[5];
  //sp 6
  sp_rates[6] += (fwd_rates[8] - rev_rates[8]) * pres_mod[5];
  ...
  //rxn 10
  //sp 0
  sp_rates[0] += (fwd_rates[10] - rev_rates[10]) * pres_mod[6];
  //sp 1
  sp_rates[1] -= 2.0 * (fwd_rates[10] - rev_rates[10]) * pres_mod[6];

  //rxn 11
  //sp 0
  sp_rates[0] += (fwd_rates[11] - rev_rates[11]) * pres_mod[7];
  //sp 1
  sp_rates[1] -= 2.0 * (fwd_rates[11] - rev_rates[11]) * pres_mod[7];

  //rxn 12
  //sp 0
  sp_rates[0] += (fwd_rates[12] - rev_rates[12]) * pres_mod[8];
  //sp 1
  sp_rates[1] -= 2.0 * (fwd_rates[12] - rev_rates[12]) * pres_mod[8];

  //rxn 13
  //sp 1
  sp_rates[1] -= (fwd_rates[13] - rev_rates[13]) * pres_mod[9];
  //sp 4
  sp_rates[4] -= (fwd_rates[13] - rev_rates[13]) * pres_mod[9];
  //sp 5
  sp_rates[5] = (fwd_rates[13] - rev_rates[13]) * pres_mod[9];
  ...
  //rxn 20
  //sp 4
  sp_rates[4] -= 2.0 * (fwd_rates[20] - rev_rates[20]) * pres_mod[10];
  //sp 7
  sp_rates[7] += (fwd_rates[20] - rev_rates[20]) * pres_mod[10];
