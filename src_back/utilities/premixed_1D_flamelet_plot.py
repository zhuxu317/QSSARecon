...
def plot_temperature_against_grid(directory_path, fig_dir):
    ...
    for file_name in csv_files:
        ...
        try:
            consumption_speed_match = re.search(r'consumption_speed_([\d]+\.[\d]+)', file_name)
            if consumption_speed_match:
                consumption_speed = float(consumption_speed_match.group(1))
                consumption_speeds.append((consumption_speed, file_name))
        except ValueError:
            continue  # Skip files that don't match the expected naming pattern
    # Sort the files by strain rate
    sorted_files = sorted(consumption_speeds, key=lambda x: x[0])

    # Create a color map
    colors = cm.jet(np.linspace(1, 0.7, len(sorted_files)))

    # Loop through all sorted files and plot them
    for idx, (consumption_speed, file_name) in enumerate(sorted_files):
        file_path = os.path.join(directory_path, file_name)
        
        # Read the CSV file
        df = pd.read_csv(file_path)
        # Check if 'normalized_grid' and 'T' columns exist in the file
        if 'normalized_grid' in df.columns and 'T' in df.columns:
            # Scatter plot T against normalized_grid with a specific color
            plt.plot(df['normalized_grid'], df['T'], color=colors[idx])
    ...
    if not sorted_files:
        ...
        for idx, (consumption_speed, file_name) in enumerate(sorted_files):
            ...
            try:
                ...
                print(f"Error plotting dataframe: {e}")
                continue
...
def plot_CEM_against_temperature(directory_path, fig_dir):
    ...
    for file_name in csv_files:
        ...
        try:
            ...
            print(f"Failed to convert {consumption_speed_match.group(1)} to float.")
            continue  # Skip files that don't match the expected naming pattern
...
def main():
    ...
    # premixed 
    directory_path = 'data/1D/test/premixed'
    fig_dir = 'figs/premixed_test'
    # non-premixed
